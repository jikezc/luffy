      ...
      <div class="user-data">
        ...
        <div class="my-order-item" v-for="order in order_list">
          ...
          <ul class="nav user-data-list" v-for="course in order.course_list">
            ...
            <li class="order-do">
              <span class="btn btn2" v-if="order.order_status==1"><router-link to="/player?vid=150f9cc41890daf12ebfa7f9817212da_1">去学习</router-link></span>
              <span class="btn btn2" v-if="order.order_status==0" @click="get_alipay_payments_url(order.order_number)">去支付</span>
            </li>
          </ul>
        </div>
      </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            get_user_order() {
                // 获取当前登录用户的所有订单
                this.$axios.get(`${this.$settings.Host}/users/orders/`, {
                    headers: {
            ...
            get_alipay_payments_url(order_number) {
                //获取支付宝支付地址
